...
| Interruption | cancel during **image** generation *(refresh mid-turn is now covered — see N22)* |
...
| N22 | **The refresh-during-image recovery was built and wired to nothing.** A turn saves a pending marker before it calls the model, and saves it *again* with the parsed reply once thinking succeeds, specifically so a refresh during image generation can finish the photo. `resumePendingTurnIfAny` (`simulation.js:3132`) does exactly that — and is **defined, exported (`:5238`) and never called by anything**. Boot only ever runs `discardInFlightTurn`, so a thinking turn you already paid for is thrown away and you retype the message. Covered by a known-red test. Wiring it up means generating an image on page load, which spends credits without a click, so the fix is an operator decision rather than a silent change | **Phase 3 — done** (offered, not automatic) |
| N23 | **A stalled thinking model was reported as an image failure.** `errors.js` had one timeout branch for the whole app, and its copy assumed an image: "image models can take several minutes… use Retry face / Retry Last Image if text already appeared." Seen live when `gemini-3.7-flash` returned empty, the engine's single retry re-sent the identical payload, and the *proxy's* 180s read timeout ended the turn — so the operator was pointed at two image buttons on a turn where no image was ever requested and no text had appeared. Nothing in the browser capped a thinking call, so Python always won the race and wrote the message. Fixed by giving `thinkingGenerate` its own 90s deadline (`THINKING_TIMEOUT_MS`, under the proxy's 180s) tagged `THINKING_TIMEOUT`, and splitting the error branch; a proxy-side timeout still routes correctly because its message names the call. A thinking timeout is deliberately *not* in `isRecoverableThinkingFail` — retrying a hang costs another 90s | **Phase 3 — done** |
| N24 | **The input-token ceiling was smaller than the prompt it was supposed to cap.** B7 made `fitInputBudget` report `overBudget` instead of silently blowing the cap, but the default of **4500** was itself the bug: the medium-density system instruction measures ~3.2k tokens for a bare character and **~4.9k for a real one**, so every play turn trimmed conversation history to *nothing* and still came in over. Worse, the budget came from the density band rather than from the setting (`INPUT_PACKS[…].tokens`), so a 6000 setting was silently rounded to 8000 **and** promoted to full density, whose instruction costs ~7k — spending the entire raise on prompt prose and leaving less room for history than 4500 did. Fixed by decoupling the two: the band picks wording, the setting is the ceiling. Default now **6000**, and the medium band runs to 6000 so the raise buys history rather than adjectives (~1.0k tokens of history for a rich character, up from none). New presets 6000 / 12000; clamp raised to 12000 | **Phase 3 — done** |
| N25 | **A stopped image was reported to the operator as a five-minute timeout.** Seen live: *"Image generation timed out / The operation was aborted."* Two independent faults produced it. `isTurnCancelled` (`simulation.js:252`) recognised only `code === 'CANCELLED'` or the message `'Turn cancelled'`, but the kie image path (`proxyFetch`) does not wrap its aborts, so the browser's raw `AbortError` — text is browser-specific, Firefox "The operation was aborted.", Chrome "The user aborted a request." — matched neither and escaped the cancel check. It then reached `classifyImageError`, whose regex lumped `abort` in with `timeout`, so a cancelled or superseded request was labelled a stall that never happened, with the raw DOMException as the body. Fixed on both sides: `isTurnCancelled(err, signal)` now counts an `AbortError` as a cancel **when this turn's own signal aborted** (an abort from anywhere else stays a failure), and `classifyImageError` gained a distinct `cancelled` reason, checked before `timeout`, with copy that says the request was stopped and her text is unaffected | **Phase 3 — done** |
| N26 | **A withheld turn was committed to history with no record of itself.** `simulation.js:4756` — the `left_on_read` / `went_quiet` / `ghost_type` branch pushed `{user, ai: '', at, mode: 'DM'}`: no `debug`, no `tracking`, and mode hardcoded, so a withhold during a Story was filed as a DM. Because the troubleshoot report diffs each turn's settings against the previous turn's, an empty record made it print **every** setting as changed, in both directions — this turn against the real one before it, and the next turn back against the empty one. Ten false "setting changed" lines at exactly the moment the operator is trying to debug something. Same shape as B5b: an early return that skips the bookkeeping the main path does. The reaction branch at `:3692` had the same omission for `thinkingModelId` and was fixed with it | **Phase 3 — done** |
| N27 | **A real image timeout ended the turn as a silent cancel.** Found while fixing N25. `interactionsCreate` builds the 5-minute timeout copy ("Nano Banana can be slow; try Nano Banana 2 Lite") and hands it to `wrapFetchError` as `context` — but that function's first branch treats *any* `AbortError` as a cancellation and returns a bare `Error('Turn cancelled')` with `code: 'CANCELLED'`, discarding the message entirely. So the B1 fix that made this path `throw` rather than `return` was still neutered one layer down: the operator saw nothing at all where they should have seen a timeout. `wrapFetchError` now takes `{timedOut}`, because an abort is not self-describing — a deadline we set and a Cancel the operator pressed arrive identically, and only the caller knows which | **Phase 3 — done** |
